...
History:
Dec 09 2015
- changed plot and Plotpath and as I moved this directory to inside CARMA/
...
Purpose
-------
plots C, D, CD, and VLA,
used for checking data collected from different array configurations
...
sym = z2        # change the sigma
CD, C, D = 'CD_USB'+sym, 'C_USB'+sym, 'D_USB'+sym

label = dict.fromkeys(['VLA', C, D, CD])
...
fvlaD = aplpy.FITSFigure(label['VLA'][0], \
        figure=figD, subplot=[x0,row_a,width,dy])
fvlaD.show_grayscale() #stretch='log', vmin=vla_min, vmax=vla_max, vmid=vla_min-0.1)

flinD = aplpy.FITSFigure(label[D][0], \
...
# Contours
########################################
fvlaC.show_contour(label['VLA'][0], colors="lime", levels=sigma_contour_array(sigma_VLA), linewidths=2, layer='fg')
fvlaC.show_contour(label[C][0], colors='red', levels=sigma_contour_CARMA(sigma_C), linewidths=2, layer='fg_cont')
flinC.show_contour(label[C][0], colors="white", levels=sigma_contour_CARMA(sigma_C), linewidths=2, layer='mol')

fvlaD.show_contour(label['VLA'][0], colors="lime", levels=sigma_contour_array(sigma_VLA), linewidths=2, layer='fg')
fvlaD.show_contour(label[D][0], colors='red', levels=sigma_contour_CARMA(sigma_D), linewidths=2, layer='fg_cont')
flinD.show_contour(label[D][0], colors="white", levels=sigma_contour_CARMA(sigma_D), linewidths=2, layer='mol')
...
# Labels
########################################
put_label(fvlaC, 0.25, 0.95, 'VLA 6GHz', 'titleBand')
put_label(fvlaC, 0.2625, 0.9, 'NA.v1.489', 'titleObj')
put_label(flinC, 0.5, 0.95, 'CARMA CO(3-2) C data'+sym[:-1], 'titleBand', c='black')
put_label(flinC, 0.31, 0.9, 'NA.v1.489', 'titleObj', c='black')

put_label(fvlaD, 0.25, 0.95, 'VLA 6GHz', 'titleBand')
put_label(fvlaD, 0.2625, 0.9, 'NA.v1.489', 'titleObj')
put_label(flinD, 0.5, 0.95, 'CARMA CO(3-2) D data'+sym[:-1], 'titleBand', c='black')
...
#        figC.savefig(Plotpath + C[:-1] + '.eps', dpi=600)
        figC.savefig(Plotpath + C[:-1] + '.png', dpi=300)
#        figD.savefig(Plotpath + D[:-1] + '.eps', dpi=600)
        figD.savefig(Plotpath + D[:-1] + '.png', dpi=300)
#        figCD.savefig(Plotpath + CD[:-1] + '.eps', dpi=600)
        figCD.savefig(Plotpath + CD[:-1] + '.png', dpi=300)
    else:
        plt.show()
